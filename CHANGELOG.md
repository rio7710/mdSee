...
- 브레드크럼 항목 클릭 시 같은 레벨 헤딩 목록 팝업 메뉴 표시 (헤딩 간 빠른 이동)
- 브레드크럼 우클릭 컨텍스트 메뉴: 현재 섹션 목차 복사 (`목차 복사하기`)
- `buildHeadingOutlineText`: 지정 레벨/섹션 범위 헤딩을 텍스트로 추출하는 유틸 함수
- `jumpToSourceLine` 에 `alignTop` 옵션 추가 (헤딩 이동 시 상단 정렬)

### Fixed

- `readColonBreak` 설정이 편집 모드에서도 복사에 적용되던 문제 수정 (읽기 모드에서만 동작)
- `parseBooleanSetting` 헬퍼를 main.js/renderer.js에 공통 적용해 설정값 파싱 일관성 개선
...
- 보기 메뉴에 `테마` 토글 추가: 다크/라이트 모드 즉시 전환 및 앱 재시작 후 유지

### Changed

- CSS 색상 체계를 확장해 `body.light-theme` 기반 라이트 팔레트 지원 (다크 기본 유지)

### Fixed

- Windows 파일 연결(더블클릭) 실행 시 전달된 `.md` 경로를 무시하고 다른(최근) 파일이 열리던 문제 수정
- 실행 중 추가 더블클릭 시 기존 창을 활성화하고 대상 파일을 즉시 로드하도록 단일 인스턴스 처리 보강
- 실행 인자 파일 확장자 허용 범위를 확장해 `json`, `xml`도 더블클릭 대상 파일로 직접 로드 가능
...
- 브레드크럼 바: 스크롤 위치에 따른 현재 헤딩 경로(h1 › h2 › h3 › h4) 실시간 표시
- 툴바 드롭다운 메뉴 재구성 (파일 / 보기 / 복사)
- 복사 드롭다운: 블록지정·HTML클립·라벨감추기·자동복사·번호삭제 토글 통합
- 줄이동 인라인 입력 필드 (숫자 입력 후 Enter 또는 이동 버튼)
- 목차 툴바 토글 버튼 및 TOC 헤더 닫기(×) 버튼
- 번호삭제/클립보드 기능: 복사 시 `1.` `1)` `(1)` `IV.` 등 번호 라벨 자동 제거 토글

### Fixed

- h5/h6 헤딩 라인 번호 미표시 버그 수정 (`annotateSourceLines` h1–h4 전용이었던 문제)
- h5/h6 포함 문서 우클릭 메뉴 미표시 버그 수정 (컨텍스트 메뉴 쿼리 h1–h6로 확장)
- h5/h6 헤딩에서 `toBullet` 실행 시 `E_SOURCE_MISMATCH` 오류 수정

### Changed

- 헤딩/리스트 레벨 하드코딩(h1–h4) → 파일 열 때 `scanDocumentStats`로 동적 감지
- `convertListItemToHeading` 블릿→헤더 변환 시 문서의 실제 `minHeadingLevel`/`maxHeadingLevel` 기준으로 클램프
- h5/h6 스타일: h4와 동일 크기(`1.1em`), 주황 계열 색상으로 시각적 구분 (h5 `#fab387`, h6 `#e07a45`)
- TOC 스크롤 동기화: 스크롤 시 활성 TOC 항목이 패널 중앙으로 자동 스크롤
